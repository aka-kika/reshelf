...
- [x] 2️⃣ ~~**Sparkle auto-update**~~ — shipped in 1.5.0 (2026-07-27) and proved by letting
      an installed 1.5.0 update itself to 1.5.1. Feed at
      `https://aka-kika.github.io/reshelf/appcast.xml` (gh-pages branch), signed with the
...
- **2026-09-23 (1.11.0: tags, folder pull, paste-to-capture)** — Shipped a sidebar **Tags** section (GitHub topics; *Most used* / *Recently added* chosen in Settings → General → Sidebar Tags; spelling twins merged via `SidebarTagRanking.key`), foldable **Folders / Categories / Tags** headings (`Section(isExpanded:)`, state in `@AppStorage`), folder right-click **Pull Repos** (check + pull only that folder's clones, same local-edit safety as ⌘U), **⌘V** paste-to-capture (`ClipboardCaptureMonitor`, only when no text field is focused), an **"Already on your shelf"** card in Quick Capture with Return → select + scroll to the repo (`revealProjectInList`), and a Quick Capture sheet that sizes to its content with a Category menu. Tried and **removed** multi-category ("also in") the same day: it inflated sidebar counts (AI / Agent 176 → 307); one main category + tags is the model. Notary credentials were re-created (the old app-specific password returned 401). Docs updated (CHANGELOG/features/README/future-features/todo).
